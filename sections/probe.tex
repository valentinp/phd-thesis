...
\section{PROBE via k-Nearest Neighbours}
...
\emph{predictors}, $\Predictor{i}{t}$.  Each predictor can be computed using both visual\footnote{Including potentially data from all four images in the pair of stereo images.} and inertial cues, allowing us to model effects like motion blur and self-similar
...
\section{Bayesian PROBE via Generalized Kernels}
...
In this section, we describe a principled, data-driven way to build a noise model
for visual odometry. We combine the previous work
on predictive robust estimation (PROBE) with work covariance estimation
...
\subsection{Mathematical Formulation}
In this section, we take the motion solution for visual odometry based on reprojection errors presented in \Cref{sec:vo_reprojection}. We build a Bayesian model for $\ImageLandmarkCovariance{i}{t}$, the covariance that defines the density of reprojection errors,
 \begin{equation}
...
To do this, we take a similar approach as before, but this time we encode a non-parametric model of covariance \textit{density} based on a dataset of errors, $\mathcal{D}$, and a predictor, $\Predictor{i}{t}$,  $p(\ImageLandmarkCovariance{i}{t}) = p(\Covariance|\mathcal{D}, \Predictor{i}{t})$. This density is then marginalized out to arrive at a posterior predictive distribution for the maximum likelihood egomotion transform.

\subsection{Inference with Ground Truth}
...
Our prediction space consisted  of the same predictors as k-NN-based PROBE: inertial magnitudes, high and low image frequency
...
(in this case: grassy self-similar textures, as well as shadows, and motion blur). 
