...
As state estimation enters the robust-perception age \citep{Cadena2016-ds}, algorithms that work in limited context will need to be adapted and modified to ensure they can operate over longer time-periods, and sundry environments. To achieve this, and to retain the benefits of classical state estimation pipelines while leveraging the representational power of modern data-drive learning techniques, we introduce the paradigm of the \textit{learned pseudo-sensor}. Instead of completely replacing the classical pipeline, this thesis presents four general ways in which machine learning can be used to train a hyper-parametric model that extracts latent information from an existing visual data stream. By fusing the output of these sensors with the output of the pipeline, we can make the final egomotion estimates more accurate in particular environments and more robust to difficult-to-model effects (\Cref{fig:intro_pseudo_sensor}) lighting changes and miscalibrations. In this thesis, we present two different ways in which this fusion can be performed. The first, which is used in Predictive Robust Estimation (PROBE, and its follow up PROBE-GK, \Cref{ch:probe}), is to use it to learn a heteroscedastic noise model that can be incorporated into a maximum-likelihood loss. This loss can then be minimized through traditional non-linear optimization techniques. The second (used by Sun-BCNN, DPC, and HydraNet,  \Cref{ch:sun-bcnn,ch:dpc,ch:hydranet}) produce geometric quantities (probabilistic estimates of an illumination source, $\LieGroupSE{3}$ corrections to existing egomotion estimates, and independent probabilistic rotation estimates, respectively), that can be fused with the original pipeline through a factor graph optimization routine.
