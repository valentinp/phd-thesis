\chapter{Learning Rotation with Uncertainty}
\label{ch:hydranet}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.68\textwidth]{so3_learning/main_fig}
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{so3_learning/hydranet_1D}
...
\subsection{Probabilistic Regression and HydraNet}
...
\end{equation}
where $w \sim \mathcal{N}(\mu=0, \sigma^2=3^2)$. Our training set consisted of 1000 samples randomly drawn from $x \in \left[0.0,0.6\right]\bigcup \left[0.8,1.0\right]$, while the test set consisted of 100 samples uniformly drawn from $x \in \left[ -2, 2\right]$. The function and the train/test samples are shown in \Cref{fig:1D_rawdata}. 
...
\begin{figure}
	...
	\centering
	\includegraphics[width=0.58\textwidth]{so3_learning/1D/nll.pdf}
	\vspace{-1em}
...
\end{equation}
As we noted in \Cref{ch:math}, unit quaternions are a double cover of $\LieGroupSO{3}$, and can represent any rotation. We choose to use this latter parametrization because of its simple analytic mean expression that we describe below.
...
\begin{wrapfigure}{r}{0.35\textwidth}
  \vspace{-20pt}
  \begin{center}
	\includegraphics[width=0.33\textwidth]{so3_learning/quat_logll}
  \end{center}
    \vspace{-20pt}
	\label{fig:math_quat_uncertainty}
...
\begin{subfigure}{0.49\textwidth}
	\centering
    \includegraphics[width=\textwidth]{so3_learning/train_summary}
    \caption{Training procedure (\Cref{alg:train_hydranet}).}
    \label{fig:hydranet_train_summary}
\end{subfigure}
~
\begin{subfigure}{0.49\textwidth}
	\centering
    \includegraphics[width=\textwidth]{so3_learning/test_summary}
    \caption{Testing procedure (\Cref{alg:test_hydranet}).}
...
\Cref{fig:sim_errors} plots rotational errors $\Vector{\phi} = \MatLog{\quat \otimes \quat_t^{-1}}$  along with 3 sigma bounds based on both the total covariance, $\Matrix{\Sigma}_t$, and the direct covariance $\Covd$. The final regression estimates have empirically consistent uncertainty, composed of a static aleatoric uncertainty  and an epistemic uncertainty (\Cref{eq:head_covariance}) that grows when the test samples come from unfamiliar input data.
...
\begin{figure*}[h!]
	...
	\begin{subfigure}[]{0.45\textwidth}
		\includegraphics[width=\textwidth]{so3_learning/kitti/kitti_abs_00_downsample_5_zoom}
		\caption{\texttt{00}}
	\end{subfigure}
	\begin{subfigure}[]{0.45\textwidth} 
