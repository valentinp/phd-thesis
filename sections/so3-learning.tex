...
Finally, we take the lessons of Sun-BCNN and DPC-Net and present a method to learn probabilistic estimates of rotation. We show that this pseudo-sensor can extract relative rotation estimates which can be fused with full pose estimates from a canonical VO pipeline estimates through pose graph optimization. To do this, we develop a network structure we call \textit{HydraNet} that can account for both epistemic and aleatoric sources of uncertainty and adapt it to the problem of estimating elements of $\LieGroupSO{3}$. This work is associated with one publication:
