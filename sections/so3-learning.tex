...
\label{ch:hydranet}
\epigraph{It is because Humanity has never known where it was going that it has been able to find its way.}{Oscar Wilde}
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\textwidth]{so3_learning/kitti/so3_learning_with_flow}
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\textwidth]{so3_learning/kitti/kitti_abs_00_downsample_5_zoom}
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\textwidth]{so3_learning/kitti/kitti_00_topdown}
...
\section{Summary}
We presented a method to regress probabilistic estimates of rotation using a deep multi-headed network structure. We used the quaternionic metric on $\LieGroupSO{3}$ to define a rotation average, and extracted anisotropic covariances by modelling uncertainty through noise injection on the manifold.
