...
that $\ImageLandmark{i}{c}$, the stereo pixel coordinates of landmark $i$ in the camera frame, is given
by 
\begin{equation}
	\ImageLandmark{i}{c} = \bbm u_l \\ v_l \\ u_r \\ v_r \ebm 
  = \ProjectionFunction(\HomogeneousPoint{i}{c}) 
  = \Matrix{M} \frac{1}{p_3}\HomogeneousPoint{i}{c},
\end{equation}
where
\begin{equation}
...
lengths and baseline of the stereo camera respectively (computed through intrinsic calibration). Note that in this
formulation, the stereo camera frame is centred between the two individual
cameras. 
