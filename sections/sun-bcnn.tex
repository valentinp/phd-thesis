\chapter{Learning Sun Direction with Uncertainty}
...
In such cases, outdoor navigation can still extract external information from measurements of celestial objects (c.f. early maritime navigation described in \Cref{ch:intro}). During the day, for example, observations of the sun can provide global orientation information since the sun's apparent motion in the sky is well-described by ephemeris models.
...
\begin{figure}
    \centering
    \includegraphics[width=0.98\textwidth]{sun-bcnn/googlenet}
...
\begin{figure*}[h]
	\centering
	\includegraphics[width=0.98\textwidth]{sun-bcnn/kitti/kitti_kitti_arrow_fig}	
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{sun-bcnn/oxford/oxford_boxwhiskers_cloudsunny_comp}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{sun-bcnn/boxwhiskers_transfer_comp}
...
The prediction paired with the uncertainty estimate allowed us to treat our model, named Sun-BCNN, as a `pseudo-sensor' of sun direction whose output could be fused with existing motion estimates much like those from a hardware-based sun sensor. In sum, our contributions with Sun-BCNN were,
\begin{enumerate}
