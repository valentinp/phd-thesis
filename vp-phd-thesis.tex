...
\title{On learning probabilistic models for state estimation}
...
\begin{notation}
    ...
    \nitem{\mbf 1 }{The identity matrix.}
    \nitem{\mbf 0 }{The zero matrix.}
    \nitem{ \mbf p_{a}^{c,b} }{ A vector from point $b$ to point $c$ (denoted by the superscript) and expressed in $\cframe{a}$ (denoted by the subscript). }
    \nitem{ \mbf C_{a,b} }{The $3 \times 3$ rotation matrix that transforms vectors from $\cframe{b}$ to $\cframe{a}$: $\mbf p_a^{c,b} = \mbf C_{a,b} \mbf p_b^{c,b}$. }
