...
\title{On learning pseudo-sensors to improve visual egomotion estimation}
...
%%%%%%%%%%%%      INCLUDE ONLY        %%%%%%%%%%%%
\includeonly{sections/introduction,sections/learning-background,sections/math}
...
Visual egomotion estimation, also known as \textit{visual odometry}, has become ubiquitous in mobile robotics due to the availability of high-quality, compact, and inexpensive cameras that capture rich representations of the world.
 To remain computationally tractable, `classical' visual odometry pipelines make simplifying assumptions that, while permitting reliable operation in ideal conditions, often lead to systematic error. In this dissertation, we present several ways in which conventional pipelines can be improved through the addition of a \textit{pseudo-sensor}. In this context, a \textit{pseudo-sensor} is a data-driven model that infers latent information that is difficult to extract through analytic measurement models. By fusing the output of such pseudo-sensors with traditional pipelines, we retain the performance of conventional techniques in nominal conditions while leveraging modern high-capacity data-driven models to improve uncertainty quantification, correct for systematic bias, and improve robustness to deleterious effects.
We validate our approach on data collected in sundry settings such as urban roads, indoor labs, and planetary analogue sites in the Canadian High Arctic.
\end{abstract}
...
\begin{notation}
    ...
    \nitem{ \Vector p_{a}^{cb}}{ A vector (resp. homogenous coodrdinates) from point $b$ to point $c$ (denoted by the superscript) and expressed in $\CoordinateFrame{a}$ (denoted by the subscript).}
    \nitem{ \Rotation_{ab} }{The $3 \times 3$ rotation matrix that transforms vectors from $\CoordinateFrame{b}$ to $\CoordinateFrame{a}$: $\Vector p_a^{cb} = \Rotation_{ab} \Vector p_b^{cb}$. }
    \nitem{ \Transform_{ba}}{ The $4 \times 4$ transformation matrix that transforms homogeneous points from $\CoordinateFrame{a}$ to $\CoordinateFrame{b}$: $\boldsymbol{p}_b^{cb} = \Transform_{ba} \boldsymbol {p}_a^{ca}$. }
\end{notation}
%\printnomenclature
...
\include{sections/learning-background}
...
\begin{appendices}
\include{sections/appendix-probe-knn}
\include{sections/appendix-vo}
\end{appendices}
